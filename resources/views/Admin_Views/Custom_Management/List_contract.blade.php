                    ...
                    <div class="table-responsive">
                        <table id="file_export" class="table table-striped table-bordered display table-hover table-dark">
                            ...
                            @foreach($contract as $key =>$contract)
                                <tr>
                                    <th>{{$contract->custom_code}}</th>
                                    <th>{{$contract->custom_contract_date}}</th>
                                    <th>{{$contract->staff_name}}</th>
                                    <th>{{$contract->custom_customer_name}}</th>
                                    <th>{{$contract->staff_id}}</th>
                                    <th>{{$contract->customer_id}}</th>
                                    <th>
                                        <?php
                                        if ($contract->custom_contract_status==1){

                                        ?>
                                        {{__('Wait for confirmation')}}
                                        <?php
                                        }
                                        else if ($contract->custom_contract_status==2){
                                        ?>
                                        {{__('Completing contracts')}}
                                        <?php
                                        }
                                        else if($contract->custom_contract_status==3){
                                        ?>
                                    ...
                                    <th style="text-align: center">
                                        <div class="btn-group">
                                            <a class="btn btn-primary" href="{{URL::to('/detail-custom-contract/'.$contract->custom_contract_id)}}"><i class="fa fa-book"></i></a>
                                        </div>
                                    </th>
                                </tr>
